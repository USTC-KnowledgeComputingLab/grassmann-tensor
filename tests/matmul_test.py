...
@pytest.mark.parametrize("impure_even", [1, 2, 3, 4, 5, 6, 7, 8])
def test_matmul_unpure_even(
    ...
    if a_is_vector:
        a = GrassmannTensor(
            (*(False for _ in broadcast_a), True if normal_arrow_order else False),
            (*((i, impure_even) for i in broadcast_a), edge_common),
            torch.randn([*[x + impure_even for x in broadcast_a], dim_common]),
        ).update_mask()
    else:
        a = GrassmannTensor(
            (*(False for _ in broadcast_a), arrow_a, True if normal_arrow_order else False),
            (*((i, impure_even) for i in broadcast_a), edge_a, edge_common),
            torch.randn([*[x + impure_even for x in broadcast_a], dim_a, dim_common]),
        ).update_mask()
    if b_is_vector:
        b = GrassmannTensor(
            (*(False for _ in broadcast_b), False if normal_arrow_order else True),
            (*((i, impure_even) for i in broadcast_b), edge_common),
            torch.randn([*[x + impure_even for x in broadcast_b], dim_common]),
        ).update_mask()
    else:
        b = GrassmannTensor(
            (*(False for _ in broadcast_b), False if normal_arrow_order else True, arrow_b),
            (*((i, impure_even) for i in broadcast_b), edge_common, edge_b),
