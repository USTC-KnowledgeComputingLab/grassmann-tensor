...
) -> tuple[NamedGrassmannTensor, NamedGrassmannTensor]:
    return request.param
...
@pytest.fixture(
    params=[
        (
            NamedGrassmannTensor(("a", "b"), (False, False), ((2, 2), (1, 3)), torch.randn([4, 4])),
            NamedGrassmannTensor(("a",), (False,), ((2, 2),), torch.randn([4])),
        ),
        (
            NamedGrassmannTensor(
                ("a", "b", "c"),
...
def mismatch_named_tensors(
    request: pytest.FixtureRequest,
) -> tuple[GrassmannTensor, GrassmannTensor]:
    return request.param
...
def test_named_tensor_arithmetic(
    unsupported_type: typing.Any,
    named_tensors: tuple[NamedGrassmannTensor, NamedGrassmannTensor],
    scalar: torch.Tensor | float,
) -> None:
    tensor_a, tensor_b = named_tensors
...
def test_named_tensor_arithmetic_fail(
    mismatch_named_tensors: tuple[NamedGrassmannTensor, NamedGrassmannTensor],
) -> None:
    tensor_a, tensor_b = mismatch_named_tensors
